...
class MixedKDJStrategy(Strategy):
    ...
    def OnTick(self):
        # 之前的挂单全撤掉
        self.engine.cancle_orders()

        target_balance, base_balance = self.engine.get_balances(self.symbol)
        ...
        close = [uitls.reserve_float(x, self.base_amount_digits) for x in df['close']]
        df['close'] = pd.Series(close)
        df['open'] = pd.Series([uitls.reserve_float(x, self.base_amount_digits) for x in df['open']])
        df['high'] = pd.Series([uitls.reserve_float(x, self.base_amount_digits) for x in df['high']])
        df['low'] = pd.Series([uitls.reserve_float(x, self.base_amount_digits) for x in df['low']])
        ...
        free_target_amount =uitls.reserve_float(target_balance['free'], self.target_amount_digits)
        if kdj_j_cur-1 > kdj_k_cur and kdj_k_cur > kdj_d_cur+1: # 开仓
            ...
            if free_target_amount > 0: # 持仓
                ...
                self.limit_buy(uitls.reserve_float(base_balance['free'], self.base_amount_digits), cur_price)
