...
class BackTest(Engine):
    ...
    def get_klines_1min(self, symbol, size, since=None):
        ...
        k1ms = self._db.find(DB_KLINES_1MIN+symbol, {"open_time":{"$gte":s_time.timestamp()*1000, "$lt":e_time.timestamp()*1000}})
        #print(k1ms[-1])
    ...
    def get_klines_1day(self, symbol, size, since=None):
        ...
        k1ds = self._db.find(DB_KLINES_1DAY+symbol, {"open_time":{"$gte":s_time.timestamp()*1000, "$lt":e_time.timestamp()*1000}})
