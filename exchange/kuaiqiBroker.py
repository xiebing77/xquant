...
class KuaiqiBroker:
    ...
    def __get_klines(self, symbol, interval, size, since):
        """获取k线"""
        # if since is None:
        #     klines = self.__client.get_klines(symbol=symbol, interval=interval, limit=size)
        # else:
        #     klines = self.__client.get_klines(symbol=symbol, interval=interval, limit=size, startTime=since)
        interval_ms = kl.get_interval_timedelta(interval).total_seconds() * 1000
        data_fileName = data_dir + "/" + symbol + "_" + interval +".csv"
        klines = []
        with open(data_fileName, "rt") as data_file:
            ...
            while True:
                try:
                    ...
                    open_time = int((int(data_values[1]) / (1000 * 1000 * 1000)) * 1000)
                    if open_time < since :
                        continue
                    close_time = open_time + interval_ms - 1
                    klines.append([open_time, float(data_values[2]), float(data_values[3]), float(data_values[4]), float(data_values[5]), int(data_values[6]), close_time])         
                    if len(klines) == size :
                        break
